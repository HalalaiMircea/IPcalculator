...
namespace Dodecahedron
{
	...
	inline Bigint::Bigint(const Bigint &b)
		: number(b.number),
		positive(b.positive),
		base(b.base),
		skip(b.skip) { }
	inline Bigint::Bigint(long long value)
	{
		base = Bigint::default_base;
		skip = 0;
		if (value < 0) {
	...
	inline Bigint::Bigint(std::string stringInteger)
	{
	...
	//Adding
	inline Bigint Bigint::operator+(Bigint const &b) const
	{
		Bigint c = *this;
		c += b;
	...
	inline Bigint &Bigint::operator+=(Bigint const &b)
	{
		if (!b.positive) {
			return *this -= b;
		}
	...
	inline Bigint Bigint::operator+(long long const &b) const
	{
		Bigint c = *this;
		c += b;
	...
	inline Bigint &Bigint::operator+=(long long b)
	{
	...
	inline Bigint Bigint::operator*(long long const &b)
	{
		Bigint c = *this;
		c *= b;
	...
	inline Bigint &Bigint::operator*=(int const &b)
	{
		std::vector<int>::iterator it = number.begin();
		long long sum = 0;
		while (it != number.end()) {
	...
	//Power
	inline Bigint Bigint::pow(int const &power, std::map<int, Bigint> &lookup)
	{
	...
	inline Bigint &Bigint::pow(int const &power)
	{
	...
	//Compare
	inline int Bigint::compare(const Bigint &a) const //0 this == a || -1 this < a || 1 this > a
	{
	...
	inline bool Bigint::operator<(Bigint const &b) const
	{
		return compare(b) == -1;
	}
	...
	//Allocation
	inline Bigint Bigint::operator=(const long long &a)
	{
		number.clear();
		long long t = a;
		do {
	...
	//Access
	inline int Bigint::operator[](int const &b)
	{
		return to_string(*this)[b] - '0';
	}

	//Trivia
	inline int Bigint::digits() const
	{
	...
	inline int Bigint::trailing_zeros() const
	{
	...
	//Input&Output
	inline std::ostream &operator<<(std::ostream &out, Bigint const &a)
	{
	...
	inline int Bigint::segment_length(int segment) const
	{
		int length = 0;
		while (segment) {
			segment /= 10;
	...
	inline Bigint abs(Bigint value)
	{
		return value.abs();
	}

	inline std::string to_string(Bigint const &value)
	{
